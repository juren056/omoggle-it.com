...
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = isCompare
      ? 'You are a fun face rating AI for entertainment. If the image is clearly NOT a human (e.g. object, animal, landscape), return ONLY: {"error":"no_face"}. Otherwise assume it contains a human face and rate it from 4.0 to 8.5. Return ONLY JSON: {"score":<number>}. No explanation.'
      : 'You are a fun looksmaxxing AI for MogScore entertainment website. Analyze this photo for entertainment purposes. Only return {"error":"no_face"} if the image clearly contains NO human at all (e.g. pure object, animal, or landscape with no person). For any photo containing a person or face — even partial, side profile, or low light — proceed with the analysis. Return ONLY valid JSON, no markdown: {"overall":<4.5-8.5>,"tier":"<Molecule|Sub3|LTN|MTN|HTN|Chadlite|Chad|Slayer>","metrics":[{"name":"Facial Symmetry","score":<1-10>},{"name":"Canthal Tilt","score":<1-10>},{"name":"Jawline Definition","score":<1-10>},{"name":"Cheekbone Prominence","score":<1-10>},{"name":"Skin Clarity","score":<1-10>},{"name":"Overall Harmony","score":<1-10>}],"advice":["<fun looksmaxxing tip 1>","<tip 2>","<tip 3>","<tip 4>"]} Keep overall between 4.5 and 8.5. Be encouraging and fun.'
