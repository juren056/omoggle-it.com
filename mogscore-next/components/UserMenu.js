...
function ProfileModal({ onClose, user }) {
  ...
  async function save() {
    ...
    setSaving(false)
    if (res.ok) { setMsg('✅ Saved! You earned 10 points for setting it up.') }
  ...
  return createPortal(
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content modal-pad" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', color: 'var(--gold)', margin: 0, letterSpacing: '.05em' }}>👤 Edit Profile</h2>
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          <label style={{ fontSize: '.88rem', color: 'var(--text-muted)', display: 'block', marginBottom: '.5rem' }}>Display Name</label>
          <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)}
            placeholder="Enter your name (2-20 chars)" maxLength={20}
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <label style={{ fontSize: '.88rem', color: 'var(--text-muted)', display: 'block', marginBottom: '.5rem' }}>Email</label>
        ...
        <div style={{ display: 'flex', gap: '.75rem' }}>
          <button onClick={onClose} style={{ flex: 1, padding: '1rem', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r-sm)', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '1rem' }}>Cancel</button>
  ...
  return (
    <>
      <div style={{ position: 'relative' }} ref={menuRef}>
        <button onClick={() => setOpen(o => !o)} className="user-menu-trigger"
          ...
          <span className="user-menu-name" style={{ fontSize: '.82rem', maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>
          <span className="user-menu-caret" style={{ fontSize: '.7rem', color: 'var(--text-muted)' }}>▾</span>
        </button>

        {open && (
          <div className="user-menu-dropdown" style={{ position: 'absolute', right: 0, top: 'calc(100% + 6px)', background: 'var(--bg2)', border: '1px solid var(--border-md)', borderRadius: 'var(--r-lg)', minWidth: 200, zIndex: 500, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,.4)' }}>
            <div style={{ padding: '.85rem 1rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '.82rem', color: 'var(--text-muted)' }}>{subscription?.isPro ? 'Pro' : 'Points'}</span>
              {subscription?.isPro ? (
            ...
            {[
              { label: '💳 Subscription', fn: () => { setShowSubscription(true); setOpen(false) } },
              { label: subscription?.isPro ? '⚙ Manage Subscription' : '⭐ Upgrade to Pro', fn: openBilling },
              { label: '◈ Points Center', fn: () => { setShowPoints(true); setOpen(false) } },
              { label: '👤 Edit Profile', fn: () => { setShowProfile(true); setOpen(false) } },
            ].map(item => (
