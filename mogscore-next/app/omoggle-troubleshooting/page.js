...
export default function TroubleshootingPage() {
  return <><Navbar /><header className="seo-page-header"><div className="container-sm"><span className="hero-eyebrow">Independent help guide</span><h1>Omoggle Troubleshooting</h1><p>Choose the symptom that matches what you see, change one thing, then repeat the check.</p><div className="highlight-box"><p><strong>Current status: unknown.</strong> Omoggle IT does not operate real-time Omoggle monitoring and will not show a fabricated green status.</p></div></div></header><main><section className="section"><div className="container-sm"><div className="troubleshooting-list">{branches.map(([title, steps, check]) => <section className="card" key={title}><div className="card-body"><h2>{title}</h2><h3>Try this</h3><p>{steps}</p><h3>Check the result</h3><p>{check}</p></div></section>)}</div><AdSlot pageType="troubleshooting" slotName="troubleshooting-after-steps" /></div></section><section className="section section-alt"><article className="content-section container-sm"><h2>Still not working?</h2><p>Record the browser name, operating system, exact error text and whether the operating-system camera app works. Then use our <Link href="/contact">contact page</Link> or Omoggle’s official support channel if one is available.</p><p>You can independently check your framing with the <Link href="/omoggle-practice-test">local camera setup tool</Link>. It does not connect to Omoggle.</p></article></section></main><Footer /></>
}
