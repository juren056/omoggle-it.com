...
Legacy global/repeated AdSense and analytics injection was removed. A centralized Adsterra-only path now maps all ten dashboard units: Native Banner, Popunder, Social Bar, Smartlink and six responsive display sizes. It requires production mode and valid per-unit configuration; sensitive, tool, account, API, legal, contact, pricing and about routes are excluded. No cookie or optional-services popup is shown. All exact GET CODE values supplied by the owner are configured and production defaults to enabled; `ADSTERRA_ENABLED=false` remains the kill switch. The unverified Google `ads.txt` line was deleted.
